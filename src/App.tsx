...
export default function App() {
  ...
  useEffect(() => {
    if (user.isLoggedIn && user.points > prevPoints) {
      ...
      setFloatingPoints(prev => [...prev, { id: newId, amount: diff }]);
      setTimeout(() => {
        setFloatingPoints(prev => prev.filter(p => p.id !== newId));
      }, 1500);
    }
    setPrevPoints(user.points);
  }, [user.points, user.isLoggedIn]);
  ...
  const getBrandDetails = (p: Platform) => {
    switch (p) {
      ...
      default: {
        ...
        return { name: `Save${str.charAt(0).toUpperCase() + str.slice(1)}`, desc: `Download Video ${str.charAt(0).toUpperCase() + str.slice(1)}` };
      }
    }
  };
    ...
    try {
      ...
      if (data) {
        ...
        if (user.isLoggedIn) {
          ...
          const trackerKey = `download_rewards_tracker_${user.email || user.username}`;
          let tracker = { date: todayStr, count: 0 };
          try {
            const savedTracker = localStorage.getItem(trackerKey);
            if (savedTracker) {
          ...
          if (tracker.count < 5) {
            ...
            const reward = settings.downloadPointsReward !== undefined ? settings.downloadPointsReward : 10;
            const updatedUser = { ...user, points: user.points + reward };
            setUser(updatedUser);
            saveUser(updatedUser);
            setSuccess(`Bonus +${reward} poin didapatkan! (Batas harian: ${tracker.count}/5)`);
          } else {
            setSuccess('Sudah mencapai batas harian, kembali besok lagi');
          }
        }
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-[2rem] p-8 max-w-md w-full text-center shadow-xl relative">
          ...
          {settings.maintenanceEndTime && remainingTime && (
            <div className="flex justify-center gap-3 sm:gap-4 mb-8">
              <FlipClockUnit label={t('Jam')} value={remainingTime.h} />
              <div className="flex flex-col items-center justify-center -mt-6">
                <div className="w-1.5 h-1.5 rounded-full bg-slate-300 dark:bg-slate-700 mb-2" />
                <div className="w-1.5 h-1.5 rounded-full bg-slate-300 dark:bg-slate-700" />
              </div>
              <FlipClockUnit label={t('Menit')} value={remainingTime.m} />
              <div className="flex flex-col items-center justify-center -mt-6">
                <div className="w-1.5 h-1.5 rounded-full bg-slate-300 dark:bg-slate-700 mb-2" />
                <div className="w-1.5 h-1.5 rounded-full bg-slate-300 dark:bg-slate-700" />
              </div>
              <FlipClockUnit label={t('Detik')} value={remainingTime.s} />
            </div>
          )}
    ...
    <div className="min-h-screen p-4 sm:p-8 font-sans flex items-center justify-center relative overscroll-none transition-colors duration-300">
      ...
      <div className="fixed top-6 right-6 sm:top-8 sm:right-8 z-40 flex items-center gap-3 sm:gap-4">
        ...
        {settings.announcement && (
          ...
          <button onClick={() => setShowProfile(true)} className="relative flex items-center gap-2 bg-white/80 dark:bg-slate-900/80 border border-slate-200 dark:border-slate-800 px-3 py-1.5 rounded-full shadow-lg backdrop-blur-md hover:scale-105 transition-all">
            ...
            <AnimatePresence>
              {floatingPoints.map(fp => (
                <motion.div
                  ...
                  animate={{ opacity: [0, 1, 1, 0], y: -45, scale: [0.8, 1.2, 1, 0.8] }}
                  exit={{ opacity: 0 }}
                  transition={{ duration: 1.4, ease: "easeOut" }}
                  className="absolute -top-4 left-1/2 -translate-x-1/2 bg-gradient-to-r from-emerald-500 to-teal-500 text-white font-extrabold text-[10px] px-2.5 py-1 rounded-full shadow-lg shadow-emerald-500/30 flex items-center gap-1 z-50 whitespace-nowrap"
                >
                  ✨ +{fp.amount} pts
                </motion.div>
              ))}
            </AnimatePresence>
            <div className="w-6 h-6 rounded-full overflow-hidden bg-indigo-100 dark:bg-indigo-900">
              <img src={user.avatar} alt="Avatar" className="w-full h-full object-cover" />
            </div>
            <div className="hidden sm:block text-left">
              <div className="text-[10px] font-bold text-slate-500 leading-none">Poin</div>
              <motion.div 
                key={user.points}
                initial={{ scale: 1 }}
                animate={user.points > prevPoints && prevPoints > 0 ? {
                  scale: [1, 1.3, 0.9, 1.1, 1],
                  color: ["#4f46e5", "#10b981", "#4f46e5"],
                  x: [0, -3, 3, -1.5, 1.5, 0]
                } : {}}
                transition={{ duration: 0.6 }}
                className="text-xs font-black text-indigo-600 dark:text-indigo-400 leading-none mt-0.5"
              >
                {user.points}
              </motion.div>
            </div>
                ...
                <div className="space-y-6">
                    ...
                    {success && (
                      <motion.div 
                        initial={{ opacity: 0, y: -10 }}
                        animate={{ opacity: 1, y: 0 }}
                        className={`text-sm font-medium px-4 py-3 rounded-xl border ${success.includes('mencapai batas') ? 'text-amber-500 bg-amber-500/10 border-amber-500/20' : 'text-emerald-500 bg-emerald-500/10 border-emerald-500/20'}`}
                      >
                        {success}
                      </motion.div>
                    )}
                    ...
                    {platform === 'youtube' && (
                      <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} className="flex flex-wrap gap-2 justify-center py-2">
                        ...
                        {['144p', '360p', '720p', '1080p', '1080p60', '4K'].map(q => (
                          <button
                            key={q}
